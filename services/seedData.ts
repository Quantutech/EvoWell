/**
 * @deprecated DEVELOPMENT SEEDING DATA
 * This file contains mock data used for:
 * 1. Initial development prototyping
 * 2. Seeding the Supabase database via api.seedDatabase()
 * 
 * DO NOT rely on this file for runtime production data.
 * Authoritative data for Users/Providers is in Supabase.
 * Content data (Blogs/Testimonials) temporarily remains here until migrated.
 */
...
const providers: ProviderProfile[] = [
...

  {
    ...
    digitalProducts: [],
    servicePackages: [
      // Fixed: Specifying required properties for ServicePackage and using correct names (price -> priceCents, active -> isActive)
      { id: 'pkg-1', providerId: `prov-${PROV_ID_1}`, name: 'Anxiety Relief Bundle', description: '4 sessions + digital workbook', priceCents: 60000, durationMinutes: 50, sessionsIncluded: 4, isActive: true },
      { id: 'pkg-2', providerId: `prov-${PROV_ID_1}`, name: 'Single Consultation', description: 'One-time assessment', priceCents: 17500, durationMinutes: 50, sessionsIncluded: 1, isActive: true }
    ],
  ...
  },
  {
    ...
    digitalProducts: [],
    servicePackages: [
      // Fixed: Specifying mandatory fields and correctly mapping property names (price -> priceCents, active -> isActive)
      { id: 'pkg-coach-1', providerId: `prov-${PROV_ID_3}`, name: '30-Day Reset', description: 'Weekly coaching + daily check-ins', priceCents: 29900, durationMinutes: 45, sessionsIncluded: 4, isActive: true }
    ],
    insuranceAccepted: [],
