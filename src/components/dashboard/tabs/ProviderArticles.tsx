...
const ProviderArticles: React.FC<ProviderArticlesProps> = ({ providerBlogs, provider, user, onRefresh }) => {
  ...
  const handleBlogAi = async () => {
    ...
    try {
      const result = await api.ai.generateBlogPost(newBlog.title, provider.professionalTitle || 'Health Professional');
      setNewBlog({ ...newBlog, title: result.title, content: result.content });
  ...
  const handleCreateBlog = async (e: React.FormEvent) => {
    ...
    try {
      await api.createBlog({
        ...newBlog,
        slug: newBlog.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') + '-' + Date.now(),
        summary: newBlog.content.replace(/<[^>]*>?/gm, '').substring(0, 150) + '...',
        ...
        isAiGenerated: aiFlags.length > 0 || newBlog.content.includes("AI-assisted"),
               ...
               <form onSubmit={handleCreateBlog} className="space-y-4 flex-grow overflow-y-auto custom-scrollbar p-1">
                  ...
                  {aiFlags.length > 0 && (
                    <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl mb-4">
                      ...
                      <p className="text-[10px] text-amber-600 mt-2">Please review and edit the content before submitting.</p>
                    </div>
                  )}
